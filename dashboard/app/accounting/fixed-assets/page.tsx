        ...
        {showCreate && (
          <div className="mb-6 p-5 rounded-xl bg-gray-900/50 border border-gray-800 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Name *</label>
              ...
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Asset Tag</label>
              ...
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Purchase Date *</label>
              ...
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">In-Service Date *</label>
                <input type="date" value={formInServiceDate} onChange={(e) => setFormInServiceDate(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 text-sm text-white [color-scheme:dark]" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Purchase Cost *</label>
                <input type="number" value={formPurchaseCost} onChange={(e) => setFormPurchaseCost(e.target.value)}
                  step="0.01" placeholder="85000.00"
                  className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 text-sm text-white" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Salvage Value</label>
                <input type="number" value={formSalvageValue} onChange={(e) => setFormSalvageValue(e.target.value)}
                  step="0.01" placeholder="5000.00"
                  className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 text-sm text-white" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Useful Life (months)</label>
                <input type="number" value={formUsefulLifeMonths} onChange={(e) => setFormUsefulLifeMonths(e.target.value)}
                  placeholder="60"
                  className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 text-sm text-white" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Depreciation Method</label>
              ...
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Linked Truck ID</label>
        ...
        {showDepreciate && (
          <div className="mb-6 p-5 rounded-xl bg-gray-900/50 border border-gray-800 space-y-4">
            ...
            <div className="flex items-end gap-4">
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Period (1st of Month)</label>
        ...
        {showDispose && selectedAsset && (
          <div className="mb-6 p-5 rounded-xl bg-gray-900/50 border border-red-900/30 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Disposal Date</label>
                <input type="date" value={disposeDate} onChange={(e) => setDisposeDate(e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 text-sm text-white [color-scheme:dark]" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Amount Received</label>
                <input type="number" value={disposeAmount} onChange={(e) => setDisposeAmount(e.target.value)}
                  step="0.01" placeholder="0.00"
                  className="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-800 text-sm text-white" />
              </div>
              <div>
                <label className="block text-[10px] text-gray-600 uppercase tracking-wider mb-1">Disposal Method</label>
        ...
        {selectedAsset && (
          <div className="mb-6 p-5 rounded-xl bg-gray-900/50 border border-gray-800 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              <div><span className="text-gray-600 text-[10px] uppercase">Category</span><p className="text-gray-300 capitalize">{selectedAsset.category}</p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Status</span><p><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${STATUS_COLORS[selectedAsset.status] || ""}`}>{selectedAsset.status.replace("_", " ")}</span></p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Purchase Cost</span><p className="text-gray-300">{fmt(Number(selectedAsset.purchase_cost))}</p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Book Value</span><p className="text-emerald-400">{fmt(Number(selectedAsset.book_value))}</p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Salvage Value</span><p className="text-gray-300">{fmt(Number(selectedAsset.salvage_value))}</p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Accum. Depr.</span><p className="text-amber-400">{fmt(Number(selectedAsset.accumulated_depreciation))}</p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Method</span><p className="text-gray-300">{selectedAsset.depreciation_method.replace(/_/g, " ")}</p></div>
              <div><span className="text-gray-600 text-[10px] uppercase">Useful Life</span><p className="text-gray-300">{selectedAsset.useful_life_months} months</p></div>
            </div>

            {/* Depreciation Schedule */}
            {selectedAsset.depreciation_entries && selectedAsset.depreciation_entries.length > 0 && (
              <div>
                <h4 className="text-[10px] uppercase tracking-wider text-gray-600 font-medium mb-2 mt-2">Depreciation Schedule</h4>
              ...
              <tbody>
                {assets.map((a) => (
                  <tr key={a.id}
                    ...
                    <td className="px-4 py-2">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${STATUS_COLORS[a.status] || ""}`}>
                        {a.status.replace("_", " ")}
                      </span>
                    </td>
