...
**PLC / Modbus Sensor module.** Deployed and working. Custom Python module at `modules/plc-sensor/` registered as Viam component `plc-monitor`. Connects via Modbus TCP to the Pi Zero W PLC simulator at `raiv-plc.local:502`. Reads 25 E-Cat cable registers (0-24), 14 sensor data registers (100-113), and push button coil 0. All register values are forced to unsigned int16 via `_uint16()` to handle pymodbus version differences. Returns E-Cat signals with named keys matching the 25-pin cable pinout, plus decoded sensor data and button state ("pressed"/"released"). Returns all-zero readings with `connected: false` on Modbus failure (2-second timeout) and reconnects automatically on next poll. Data capture at 1 Hz, readings sync to Viam Cloud. The Pi Zero W (192.168.0.173) runs the PLC simulator as a systemd service on port 502, reading physical GY-521 accelerometer, DHT22 temperature/humidity sensors, and a Fuji AR22F0L E3 push button on GPIO 17. The Pi 5 (192.168.0.172) runs viam-server with the plc-sensor module connecting across the shop network.
